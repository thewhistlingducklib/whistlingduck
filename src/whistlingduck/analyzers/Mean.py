...
def Mean(dataset: Any, 
    ...
    """
    Calculate mean values for specified numeric columns in a dataset using DuckDB.
    
    This function calculates the mean value in specified columns, ensuring they are numeric.
    ...
    Returns:
        List[Dict[str, Union[str, float, dict, None]]]: A list of dictionaries with the following keys:
            ...
            - mean_value (float): Mean value in the column
    ...
    Args:
        ...
        filter_condition_dict (Optional[Dict[str, Union[str, int, float]]], optional):
            Dictionary of filter conditions to apply before calculating mean.
        ...
        decimal_places (int, optional): Number of decimal places to round the mean value. 
            Defaults to 2.
    """
    ...
    temp_table_name = f"mean_{unique_id}"
    ...
    for column in final_column_list:
        ...
        if not any(numeric_type in column_type for numeric_type in numeric_types):
            raise ValueError(
                f"Column '{column}' is of type {column_type}. Mean operation requires numeric columns. "
    ...
    sql_statements = [
        ...
        SELECT 
            ...
            ROUND(CAST(AVG({column}) AS DOUBLE), {decimal_places}) as mean_value
        FROM {source_table}
        {where_clause}
        """
        for column in final_column_list
